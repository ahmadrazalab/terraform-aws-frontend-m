...
resource "aws_cloudfront_distribution" "dashboard_distribution" {
  origin {
    ...
    origin_path = "/dash" # Set this to the folder containing your static files
  }

  enabled             = true
  is_ipv6_enabled     = true
  comment             = "CloudFront Distribution for S3 Static Website"
  default_root_object = "dash/index.html" # change according to application configuration 
  ...
  tags = {
    Name = "CloudFront Distribution for dashboard "
  }
}
